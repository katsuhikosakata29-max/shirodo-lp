...
- 取得後、`metrics/` の変更は **自動で main にコミット&プッシュされる**(`--no-git` で無効化)。
  - main 以外のブランチをチェックアウト中はスキップ(次に main で実行されたとき反映)
  - push失敗(オフライン等)はコミットだけ残し、翌日以降のpushで回収される
  - `metrics/` 以外の未コミット変更には触れない
