...
色・書体・余白・コンポーネントの正は `docs/DESIGN.md`。新ページ・既存ページの改修・生成スクリプトの見た目変更は、必ず先にこれを読み、書かれた値と「やらないこと」に従う。トークンを足す・変えるときは DESIGN.md を先に更新し、全ページの `:root` と `scripts/gen_*.py` に反映する（各ページはトークンをインラインで複製しているため、1 箇所だけ直すとズレる。実際に `/guide/level/` の朱だけ `#b0402c` にズレていた事故がある。2026-09-05 修正）。他社サイト由来の DESIGN.md（refero 等）は取り込まない。
