...
- **共通のスタイルは `assets/site.css`**（色の変数、リセット、ヘッダー、フッター、パンくず、目次、FAQの一部、アプリ紹介枠など）。各ページの `<style>` より前に読み込まれ、同じセレクタはページ側が上書きする。読み込みURLの `?v=` はCSSの中身から自動で決まるので、CSSを変えたら `sync_shared.py` と生成ページの再生成が必要（コミット前フックとテストで検出される）
- ヘッダー・ボタン・フッターは読み物ページの見た目が共通CSSの標準。**トップだけ違う見た目**はトップの `<style>` で上書きしている（塗りボタンは `border: 0` で枠線を打ち消している）
- 共通CSSに入れていない規則: `p` などの要素名だけの規則（持っていないページにも効いてしまうため）と、見出しの大きさや本文の幅などページごとに値が違う規則。統一するなら見た目の判断が必要
